...
const RealtimeConnectionBar = ()=>{
    ...
    //keep local input in sync with connected url
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="flex px-2 flex-col items-end text-xs text-zinc-400">
                    <div className="flex items-center gap-1">
                        <div
                        className={cn("w-2 h-2 rounded-full", status==='connected'? "bg-green-500": status==='connecting'|| status==='reconnecting'? 'bg-yellow-500 animate-pulse':status==='error'?'bg-red-500':'bg-zinc-500')}
                        />
                ...
                onClick={onConnect}
                disabled={status==='connecting' || status ==='reconnecting'}
                className={cn('ml-2 text-white font-bold transition-colors', getConnectionColor())}
                >
