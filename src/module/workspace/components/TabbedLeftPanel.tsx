            ...
            <div className="w-12 bg-zinc-900 border-r border-zinc-800 flex flex-col items-center py-4 space-y-4">
                {sidebarItems.map((item,index)=>(
                    <Hint label={item.label} key={index} side='right'>
                        ...
                        href={item.link}
                        key={index}
                        className={cn('w-8 h-8 rounded-lg flex items-center justify-center cursor-pointer transition-colors', activeTab===item.label ?'bg-indigo-600 text-white':'text-zinc-400 hover:text-zinc-300 hover:bg-zinc-800')}
                        >
